 function generate_model_unit_square(nk;simplexify_model=true)
  ...
  function generate_model_unit_square_biot_stokes(nk;
                                                      simplexify_model=true,
                                                      bcs_type=:mixed)
    domain    = (0.0,1.0,0.0,1.0)
    n         = 2^nk
    partition = (n,2*n)
    m=generate_rectangular_biot_stokes_mesh(:t,
                                                 n,
                                                 [:b],
                                                 domain,
                                                 partition,
...
function setup_model_labels!(model::Gridap.Geometry.UnstructuredDiscreteModel,bcs_type::Symbol)
    @assert bcs_type in (:full_dirichlet,:mixed) 

    if (bcs_type==:mixed)
      # Define Dirichlet/Neumann boundaries
      #   5  11   6
      #   12  P  13
      #   3   8   4
      #   9   E  10
      ...
      add_tag!(labels,"wallED",[1,2,4,7,10])
      add_tag!(labels,"wallEN",[1,3,9])
      add_tag!(labels,"wallPD",[3,5,6,12,11])
      add_tag!(labels,"wallPN",[6,13,4])
    else
      labels = get_face_labeling(model)
      add_tag!(labels,"wallED",[1,2,4,7,10,3,9])
      add_tag!(labels,"wallEN",Int[])
      add_tag!(labels,"wallPD",[3,5,6,12,11,13,4])
      add_tag!(labels,"wallPN",Int[])
    end 
 end
...
function generate_rectangular_biot_stokes_mesh(block,
                                                   ncells_stokes_region_thin_dim,
...
function generate_vessel_interstitium_model_block(block,
    ...
    if block==:t 
      ...
      partition=(NX,NY)
      generate_rectangular_biot_stokes_mesh(block,
                                                vessel_ncells_in_thin_dim,
                                                vessel_location,
                                                domain,
                                                partition,
