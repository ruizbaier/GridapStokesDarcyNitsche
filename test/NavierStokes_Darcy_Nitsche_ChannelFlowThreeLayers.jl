...
# parabolic profile between y = -0.2 and y = 1.2
uin(x) = VectorValue(0.01*tanh(40*(x[2]+0.2)*(1.2-x[2])),0)

ff = VectorValue(0,0)
fp = VectorValue(0,0)
v0 = VectorValue(0,0)

model = GmshDiscreteModel("meshes/ComplexChannelWithThreeLayers.msh")
writevtk(model,"meshes/ComplexChannelWithThreeLayers")
...
# FE spaces
Vf  = TestFESpace(Ω_f, reffe_uf, conformity=:H1, dirichlet_tags=["FluidIn","FluidObstacles"])
...
writevtk(Ω_f,"NavierStokesDarcy_NewChannel_fluid",order=1,cellfields=["uf"=>ufh,"pf"=>pfh])
writevtk(Ω_p,"NavierStokesDarcy_NewChannel_porous",order=1,cellfields=["up"=>uph,"pp"=>pph])
