...
rhs((vf,vp,qf,qp)) = lvf(vf) + lvp(vp) 

op = AffineFEOperator(lhs,rhs,X,Y)            
ufh, uph, pfh, pph = solve(op)

writevtk(Ω_f,"StokesDarcy_Channel_fluid",order=1,cellfields=["uf"=>ufh,"pf"=>pfh])
writevtk(Ω_p,"StokesDarcy_Channel_porous",order=1,cellfields=["up"=>uph,"pp"=>pph])
